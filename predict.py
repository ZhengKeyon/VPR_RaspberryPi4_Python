...
def cv2ImgAddText(img, text, left, top, textColor=(0, 255, 0), textSize=20):
    ...
    fontStyle = ImageFont.truetype(
        "lib_E-ink/pic/Font.ttc", textSize, encoding="utf-8")    # 这里换成自己字体文件的目录
    # 绘制文本
    ...
    def predict(self, car_pic):
        ...
        for card_index, card_img in enumerate(card_imgs):            # 对提取的每个车牌区域进行识别
            ...
            if card_img_hsv is None:
                continue
            row_num, col_num = card_img_hsv.shape[:2]
            card_img_count = row_num * col_num      # 我猜是整个区域的像素点数，瞎猜的哈哈
        ...
        for i, color in enumerate(colors):
            if color in ("blue", "yello", "green"):
                ...
                finalimg = cv2ImgAddText(oldimg, LRPresult.decode('utf-8'), 50, 50, (0, 0, 255), 40)
                cv2.imshow("树莓派车牌识别", finalimg)
                roi = card_img
                card_color = color
                break
